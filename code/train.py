...
def evaluate_on_split(model, df):
    """
    Runs the model on df deterministically and returns mean episode reward.
    Uses a single episode covering the full split.
    """
...
hyperparams_grid = [
    {"learning_rate": 3e-4, "ent_coef": 0.01,  "n_steps": 2048},
    {"learning_rate": 1e-4, "ent_coef": 0.01,  "n_steps": 2048},
    {"learning_rate": 3e-4, "ent_coef": 0.001, "n_steps": 1024},
]


def train_fold(fold_config, df, total_timesteps=300_000, seed=42):
    ...
    for i, hp in enumerate(hyperparams_grid):
        env = Monitor(BTCTradingEnv(train_df))
        model = PPO(
            "MlpPolicy",
            env,
        ...
        val_reward = evaluate_on_split(model, val_df)
    ...
    ax.set_title(f"Training Curve - Fold {fold_num}  (best lr={best_hp['learning_rate']}, ent={best_hp['ent_coef']})")
...
def run_training(total_timesteps=300_000, seed=42):
    df = load_and_prepare()

    trained = []
    for fold_config in folds:
        result = train_fold(fold_config, df, total_timesteps=total_timesteps, seed=seed)
